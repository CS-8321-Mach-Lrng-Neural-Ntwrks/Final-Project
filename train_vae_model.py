...
def main_vae():
    ...
    # --- Generate synthetic samples and save as .npy ---
    vae_model.eval()
    with torch.no_grad():
        z = torch.randn(100, LATENT_DIM).to(device)
        synthetic = vae_model.decode(z).cpu().numpy()

    output_path = "vae_model_output/output.npy"
    np.save(output_path, synthetic)
    logging.info(f"VAE output saved to {output_path}")
