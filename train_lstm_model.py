...
def main():
    ...
    with torch.no_grad():
        input_seq = torch.randn(100, SEQUENCE_LENGTH, len(FEATURE_COLUMNS)).to(device)
        synthetic_output = model(input_seq)  # (100, 1) if using last timestep only
        synthetic_output = synthetic_output.cpu().numpy()

    np.save("pytorch_lstm_model_output/output.npy", synthetic_output)
    print(f"LSTM output saved to pytorch_lstm_model_output/output.npy, shape: {synthetic_output.shape}")
